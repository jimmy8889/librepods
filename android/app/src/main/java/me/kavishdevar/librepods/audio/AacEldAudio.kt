...
object AacEldPacketParser {
    ...
    fun isAudioPacket(packet: ByteArray): Boolean =
        packet.size >= 8 &&
            packet[0] == 0x04.toByte() && packet[1] == 0x00.toByte() &&
    ...
    fun parse(packet: ByteArray): List<AacEldAccessUnit> {
        ...
        while (offset + 5 <= packet.size) {
            ...
            if (length == 0 || end > packet.size) break
...
class AacEldDecoder(
    ...
    fun start(): Boolean {
        ...
        check(started.compareAndSet(false, true)) { "Create a new decoder for each session" }
        running.set(true)
        return try {
            val created = codecFactory()
            codec = created
            created.also { decoder ->
    ...
    private fun decodeLoop() {
        ...
        try {
            while (running.get()) {
                ...
                var inputIndex = decoder.dequeueInputBuffer(CODEC_TIMEOUT_US)
                while (inputIndex < 0 && running.get()) {
                    drain(decoder)
                    inputIndex = decoder.dequeueInputBuffer(CODEC_TIMEOUT_US)
                }
                if (inputIndex >= 0) {
            ...
            when (val outputIndex = decoder.dequeueOutputBuffer(info, 0)) {
                ...
                MediaCodec.INFO_OUTPUT_FORMAT_CHANGED -> Unit
                    ...
                    decoder.getOutputBuffer(outputIndex)?.let { output ->
                        ...
                        val encoding = if (format.containsKey(MediaFormat.KEY_PCM_ENCODING)) format.getInteger(MediaFormat.KEY_PCM_ENCODING) else android.media.AudioFormat.ENCODING_PCM_16BIT
                        check(encoding == android.media.AudioFormat.ENCODING_PCM_16BIT) { "Unsupported PCM encoding $encoding" }
                        listener.onPcmData(
                            pcm,
